...
namespace ConverterProgram.Conversions {
    ...
    internal abstract class ACurrencyHandler {
        ...
        /// <exception cref="ArgumentException">if the number of units is greater 999,999,999 and the number of subunits is greater than 99</exception>
        public string ConvertCurrencyToWords(string units, string? subunits) {
            if (units.Length > ConversionsConstants.MAX_DIGITS_UNIT) {
                throw new ArgumentException(string.Format(EXC_MSG_MAX_NUMBER_EXCEEDED_TF, GetUnitsName(), units.Length, ConversionsConstants.MAX_DIGITS_UNIT));
            }
            // account for units
            ...
            if (subunits != null) {
                ...
                if (subunits.Length > ConversionsConstants.MAX_DIGITS_SUBUNIT) {
                    throw new ArgumentException(string.Format(EXC_MSG_MAX_NUMBER_EXCEEDED_TF, GetSubunitsName(), subunits.Length, ConversionsConstants.MAX_DIGITS_SUBUNIT));
                }
